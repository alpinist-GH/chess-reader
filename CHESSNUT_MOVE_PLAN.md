...
#### `motionProtocolVerified` flipped to `true` (2026-09-08)

`UniversalBleTransport.motionProtocolVerified` now returns `true`, citing the macOS completion/stop/interruption/piece-presence findings above plus all-four-platform MTU/service verification.

Flipping the flag alone would not have enabled real movement: `_sendTargetCommand` in `chessnut_controller.dart` also refuses to send unless `_transport.availablePieces` is non-null, and `UniversalBleTransport.availablePieces` was still hardcoded `null` — never wired to the live piece-status query the controller already polls periodically. Wired it up:

- `ChessnutTransport.availablePieces` is now settable (`set availablePieces(...)` added to the interface), not just a getter.
- `UniversalBleTransport` exposes it as a plain mutable field (default `null`), cleared back to `null` on `disconnect()`.
- `ChessnutController._handleIncomingCommandResponse` computes an availability map from every decoded piece-status response — `(0, 0)` (the confirmed absence sentinel) excluded, every other coordinate (including an off-board reserve/holder slot) counted — and pushes it to `_transport.availablePieces`. `_onDisconnected()` also resets it to `null`, matching the existing `clearPieceStatuses` reset at the same site, so availability is always re-verified per connection rather than assumed to persist.
- `FakeChessnutBoard` gained `respondToPieceStatusQuery` (simulate a query that never answers, keeping availability unknown) and `setPieceAvailability(pieceChar, available)` (simulate specific pieces reported absent via realistic `(0, 0)` coordinates) so tests exercise the real decode path instead of poking the transport field directly. Updated `chessnut_regression_test.dart`'s "every motion entry point rejects unverified inventory/missing piece" cases to use these instead of directly setting `board.availablePieces`, since the controller now overwrites that field on every real piece-status response and the old direct-set technique was immediately undone by the connect-time query.

`flutter analyze` and the full test suite pass (same pre-existing `engine_test.dart` failure noted above, unrelated — missing local Stockfish binary asset).

Not yet done: repeating the Stop/interruption/piece-presence protocol trials on iOS, Android, and Windows hardware — those were only ever exercised on macOS, and the plan's confirmed findings are still macOS-specific even though the flag is now global across platforms. This is deliberate, not an oversight: per the plan's own earlier notes (iOS and Android sections above), this protocol behavior is BLE-hardware/firmware-level, not platform-level — per-platform sessions are scoped to MTU/GATT negotiation only, and that scoping is what determined `motionProtocolVerified`'s flip, not a per-platform re-run of the macOS trials.

#### Codex review of the above, and what changed as a result (2026-09-08)

Ran a Codex review against the diff. Two findings were real bugs and got fixed; one finding's suggested fix was tried, found to conflict with an existing deliberate test/design decision, and reverted; the platform-scoping finding was already addressed by the note above.

- **Fixed: a malformed/truncated piece-status response could silently keep stale availability data.** `_handleIncomingCommandResponse` now checks the packet header (`41 89 0b`) independently of `ChessnutCodec.decodePieceStatusResponse`'s own length check, so a header-matching-but-truncated packet (e.g. a partial delivery) explicitly sets `_transport.availablePieces = null` — treated as unverified — rather than leaving whatever was last known.
- **Tried and reverted: trusting each entry's own per-piece "identity" byte instead of its position in the packet.** Codex's premise was that positional decoding could silently mislabel pieces if the firmware ever reorders entries. Implemented it (map by `packet[offset]` instead of loop index, reject on missing/duplicate/out-of-range identities), but it broke `chessnut_codec_test.dart`'s `decodePieceStatusResponse` test, which uses placeholder identity-byte values (`1` and `12`) that don't follow a sequential 1–34 scheme and were never asserted to mean anything — i.e. the existing test (and the original implementation it was written against) already encodes the position-based design, not an identity-based one. More importantly, the milestone log's "plausible x/y coordinates ... across all 34 nominal pieces" finding was itself obtained by reading the real hardware response *with the positional decoder* — that's the actual empirical evidence, and it's stronger than Codex's untested reordering hypothesis. Reverted to positional decoding; the identity byte's real meaning remains unconfirmed and out of scope until a future hardware session actually investigates it.
- **Fixed, then redesigned after breaking tests: piece-availability staleness before authorizing motion.** First attempt inserted an `await` (a fresh piece-status query/response round trip) directly into `_sendTargetCommand`, the path every rapid book-navigation position change goes through. This broke three regression tests (`rapid navigation sends only latest pending target after completion`, `navigation back to in-flight target clears stale pending target`, `pause discards in-flight completion and pending positions`) because that path's correctness depends on claiming `_inFlightTargetPlacement` *synchronously* — back-to-back position changes in the same tick must see the claim already made and queue as pending, and `pause()` interleaving must not be able to race a claim that hasn't sent yet. Landed instead as `_refreshAvailablePieces()`, called only from `startOrResume()` and `sendDiagramAnyway()` — the two deliberate, infrequent, user/session-triggered entry points into synchronization, not the high-frequency per-move dispatch path. This narrows (does not eliminate) the staleness window Codex identified: a piece removed mid-session, between an explicit Resume and the next 5-minute `batteryPollInterval` tick, can still be authorized by rapid navigation during that window. Accepted rather than further engineered, given: (a) the failure mode is bounded and already caught — the board simply can't complete a move to a missing piece, and the existing 30-second `_motionTimer` pauses with an honest "Movement completion timed out" error; (b) forcing a round trip onto the hot path repeatedly proved to fight the synchronous single-flight/pending-collapse design this integration deliberately relies on for rapid navigation.
