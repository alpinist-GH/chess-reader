...
void main() {
  ...
  test(
    'grades the next resolved book move and restores a wrong attempt',
    () async {
      ...
      expect(container.read(proTrialRemainingProvider), 4);
  ...
  test('does not start without Pro entitlement', () {
    final container = ProviderContainer(
      overrides: [proEntitlementProvider.overrideWithValue(false)],
    );
    addTearDown(container.dispose);
    ...
    container.read(guessMoveProvider.notifier).start();
    expect(container.read(guessMoveProvider).phase, GuessMovePhase.idle);
  });
}
